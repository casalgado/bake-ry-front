...
const flattenedOrderItems = ref([]);

const updateFlattenedOrderItems = (retryCount = 0) => {
  console.log('items before flattening:', orderStore.items);
  console.log('first order orderItems:', orderStore.items[0]?.orderItems);

  // Check if all orders have valid orderItems before updating
  const hasValidOrderItems = orderStore.items.every(order =>
    order.orderItems && Array.isArray(order.orderItems),
  );

  if (!hasValidOrderItems && retryCount < 3) {
    console.log(`OrderItems not ready yet, refetching data... (attempt ${retryCount + 1}/3)`);
    setTimeout(async () => {
      ...
      updateFlattenedOrderItems(retryCount + 1);
    }, 1000);
    return;
  }

  if (!hasValidOrderItems) {
    console.error('OrderItems still not ready after 3 retries, skipping update');
    return;
  }

  flattenedOrderItems.value = orderStore.items
    .flatMap(order =>
...
const uniqueCollections = ref([]);

const updateUniqueCollections = () => {
  // Check if all orders have valid orderItems before updating
  const hasValidOrderItems = orderStore.items.every(order =>
    order.orderItems && Array.isArray(order.orderItems),
  );
  ...
  if (!hasValidOrderItems) {
    console.log('UniqueCollections - OrderItems not valid, skipping update');
    return;
  }
  ...
  uniqueCollections.value = Array.from(collections);
};
...
const handleToggleUpdate = async ({ rowIds, field, value }) => {
  try {
    ...
    // Update flattened items
    updateFlattenedOrderItems();
    updateUniqueCollections();
  } catch (error) {
...
watch(
  ...
  async (newRange) => {
    try {
      ...
      // Update flattened items after fetching new data
      updateFlattenedOrderItems();
      updateUniqueCollections();
    } catch (error) {
...
onMounted(async () => {
  try {
    ...
    // Initialize flattened order items
    updateFlattenedOrderItems();
    updateUniqueCollections();
  } catch (error) {
