...
<template>
  <div class="flex items-center gap-2">
    ...
    <div ref="searchContainer" class="flex items-center">
      <button @click="toggleSearch" class="p-2 hover:bg-neutral-100 rounded-lg"
        :class="{ 'bg-neutral-100': showSearch }">
        <PhMagnifyingGlass class="w-5 h-5" />
      </button>

      <div v-if="showSearch" class="ml-2 overflow-hidden">
        <input ref="searchInput" :value="search" @input="handleSearchInput" type="text"
    ...
    <div class="flex items-center gap-2 flex-grow justify-end">
        ...
        <template v-for="filter in filters" :key="filter.field">
          <div class="flex items-center gap-1">
            <template v-for="option in filter.options" :key="option.value">
              <button @click="$emit('toggle-filter', filter.field, option.value)"
                class="px-2 py-1.5 rounded-lg text-sm font-medium transition-colors" :class="[
      ...
      <div class="" v-else>
        <Menu as="div" class="relative inline-block text-left" v-slot="{ open }">
          <MenuButton class="inline-flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-neutral-100"
            ...
            <span v-if="activeFilterCount" class="bg-primary-100 text-primary-600 px-2 py-0.5 rounded-full text-xs">
              {{ activeFilterCount }}
            </span>
          </MenuButton>
              ...
              <div v-for="filter in filters" :key="filter.field" class="p-4">
                <div class="space-y-1">
                  <div v-for="option in filter.options" :key="option.value"
                    ...
                    <div class="w-4 h-4 border rounded flex items-center justify-center" :class="[
                      isOptionSelected(filter.field, option.value)
                        ? 'bg-primary-600 border-primary-600'
                        : 'border-neutral-300'
                    ]">
                      <PhCheck v-if="isOptionSelected(filter.field, option.value)" class="w-3 h-3 text-white" />
                    </div>
                    {{ option.label }}
                  </div>
                </div>
      ...
      <button @click="clearAll"
        class="flex items-center gap-1 px-2 py-1.5 rounded-lg text-sm font-medium transition-colors" :class="[
          hasActiveFilters
            ? 'bg-neutral-100 hover:bg-neutral-200 text-neutral-700'
